...
const UserCard = ({ user }) => {
  ...
  return (
    <button className="px-4 py-3 flex items-center hover:bg-[#ffffff08] transition-colors">
      <img
        src={user.avatar}
        alt={user.username}
      ...
      <div className="flex-1 max-w-full flex flex-col text-left">
        <div className="text-[15px] leading-5 font-bold text-[#e7e9ea] flex items-center">
          {user.fullName}
          {user.verified && (
            <svg
              viewBox="0 0 22 22"
              className="text-[#1d9bf0] inline-block ml-0.5"
              fill="currentColor"
              width={18.75}
              height={18.75}
            >
        ...
        <div className="text-[15px] leading-5 font-normal text-[#71767b]">
          @{user.username}
        </div>
      </div>
      {following ? (
